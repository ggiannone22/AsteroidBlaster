...
class Player(CircleShape): # Player class
    ...
    def update(self, dt: float) -> None:                            # Everytime a specific movement key is pressed, the corresponding movement happens
        ...
        self.shot_cooldown -= dt                                    # Subtracts cooldown by the change in time everyime player updates
    ...
    def shoot(self) -> None:     
        if self.shot_cooldown <= 0:                                 # If cooldown did not reach 0 yet
            shot = Shot(self.position.x, self.position.y)           # Allows player to shoot; Creates a shot instance using the position of player, creates a unit vector,
            unit_vector = pygame.Vector2(0,1)                       # Rotates the vector to face the player's direction, updates the velocity with PLAYER_SHOT_SPEED
            rotate_with_player = unit_vector.rotate(self.rotation)  
            shot.velocity = rotate_with_player * PLAYER_SHOOT_SPEED
            self.shot_cooldown = PLAYER_SHOOT_COOLDOWN_SECONDS      # Resets shot_cooldown
